...
/*
 * Some custom functions added to the Peter Fleury's interrupt-driven IO library
 */
extern int uart_putchar(char c, FILE *stream);	// Send a single character to uart
extern int kbhit(void); // Check if there's any characters that have been received from the uart
extern int uart_gets(char *buf, int len, unsigned char localecho); // Receive a null terminated string from the uart
